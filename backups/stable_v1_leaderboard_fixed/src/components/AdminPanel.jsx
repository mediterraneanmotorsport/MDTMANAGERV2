...
const AdminPanel = ({ onClose, circuits, cars }) => {
    ...
    const handleDeleteLeaderboardRecord = async (id) => {
        if (!window.confirm("¿Seguro que quieres borrar este tiempo del Leaderboard Global?")) return;
        setLoading(true);
        try {
    ...
    return (
        <div className="w-full h-full bg-racing-carbon flex flex-col overflow-hidden">
                ...
                <div className="flex bg-black/40 border-b border-white/5 p-1 gap-1">
                    {['setups', 'circuits', 'cars', 'manage-setups', 'leaderboard'].map(tab => (
                        <button
                            key={tab}
                            onClick={() => {
                                setActiveTab(tab);
                            ...
                            {tab === 'setups' ? 'Subir Setups' :
                                tab === 'manage-setups' ? 'Gestionar Setups' :
                                    tab === 'circuits' ? 'Circuitos' : 
                                        tab === 'leaderboard' ? 'Leaderboard' : 'Vehículos'}
                        </button>
                    ))}
                </div>
